...
Esto nos dice que E es subcuerpo de \(\mathbb{C}^{}\). Además, este cuerpo E es isomorfo a \(\mathbb{R}^{}\) (isomorfo en el sentido de cuerpo) mediante la identificación \((a,0) \in E \leftrightarrow a \in \mathbb{R}^{}\) % Falta cambiar flecha.
...
% Grafica sobre numeros complejos y sus conjugados.
\begin{center}
\begin{tikzpicture}
    \begin{axis}[
        xmin=-2,xmax=2,
...
\begin{enumerate}
  \item \(|z| \geq 0 \)
  % \item \( |z| = 0 \Leftrightarrow (Re(z))^2+(Im(z))^2= 0 \Leftrightarrow Re(z) = 0 \and Im(z) = 0 \Leftrightarrow z = 0 \) 
  \item Desigualdad triangular %Anotacion 1
